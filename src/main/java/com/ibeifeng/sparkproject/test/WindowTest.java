...
public class WindowTest {
  ...
  public static void main(String[] args) {
    SparkConf conf = new SparkConf().setMaster("local").setAppName("WindowTest");
    JavaSparkContext sc = new JavaSparkContext(conf);
    HiveContext sqlContext = new HiveContext(sc.sc());

    List<Tuple2<String, Integer>> grades = Arrays.asList(new Tuple2<String, Integer>("class1", 80),
        new Tuple2<String, Integer>("class1", 75), new Tuple2<String, Integer>("class1", 90),
    ...
    StructType schema = DataTypes
        .createStructType(Arrays.asList(DataTypes.createStructField("class", DataTypes.StringType, true),
    ...
    DataFrame gradeLevelDF = sqlContext.sql("SELECT " + "class," + "grade,"
        + "row_number() OVER(PARTITION BY class ORDER BY grade DESC) rank " + "FROM grades");
